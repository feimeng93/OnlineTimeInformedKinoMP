...
class MPCController(Controller):
    ...
    def __init__(
        ...
        if self.q_d.ndim == 2:
            ...
            self.q_d = np.hstack(
                [self.q_d, np.transpose(np.tile(self.q_d[:, -1], (self.N + 1, 1)))]
            )
    ...
    def build_objective_(self):
        ...
        if not self.add_slack:
            self._osqp_P = sparse.block_diag(
                [
                    sparse.kron(sparse.eye(self.N), CQC),
                    CQNC,
        ...
        if xr.ndim == 1:
            if not self.add_slack:
                self._osqp_q = np.hstack(
                    [
                        np.kron(np.ones(self.N), -QCT.dot(xr)),
                        -QNCT.dot(xr),
            ...
            if not self.add_slack:
                self._osqp_q = np.hstack(
                    [
                        np.reshape(-QCT.dot(xr), ((self.N + 1) * self.nx,), order="F"),
                        np.tile(2 * self.R.dot(self.const_offset), (self.N)),
                    ]
                )
            else:
    ...
    def build_constraints_(self):
        ...
        Ax = sparse.kron(sparse.eye(self.N + 1), -sparse.eye(self.nx)) + sparse.kron(
            sparse.eye(self.N + 1, k=-1), self._osqp_Ad
        )
        Bu = sparse.kron(
        ...
        if not self.add_slack:
            Aineq = sparse.block_diag(
                [self.C for _ in range(self.N + 1)] + [np.eye(self.N * self.nu)]
            )
            ...
            Aineq = sparse.hstack(
                [
                    ...
                    sparse.vstack(
                        [
                            sparse.eye(self.ns * (self.N + 1)),
                            sparse.csc_matrix(
                                (self.nu * self.N, self.ns * (self.N + 1))
                            ),
                        ]
                    ),
                ]
            ...
            Aeq = sparse.hstack(
                [
                    Ax,
                    Bu,
                    sparse.csc_matrix((self.nx * (self.N + 1), self.ns * (self.N + 1))),
                ]
            )
    ...
    def eval(self, x, t):
        ...
        if self.q_d.ndim == 2:
            ...
            self._osqp_q[: self.nx * (self.N + 1)] = np.reshape(
                -QCT.dot(xr), ((self.N + 1) * self.nx,), order="F"
            )
        ...
        return self._osqp_result.x[
            self.nx * (self.N + 1) : self.nx * (self.N + 1) + self.nu
        ]

    def parse_result(self):
        return np.transpose(
            np.reshape(
                self._osqp_result.x[: (self.N + 1) * self.nx], (self.N + 1, self.nx)
            )
        )
    ...
    def get_control_prediction(self):
        return np.transpose(
            np.reshape(
                self._osqp_result.x[
                    self.nx * (self.N + 1) : self.nx * (self.N + 1) + self.nu * self.N
                ],
                (self.N, self.nu),
            )
        )
