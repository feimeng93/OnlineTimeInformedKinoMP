...
class KoopmanNet(nn.Module):
    ...
    def construct_encoder_(self):
        ...
        if hidden_depth > 0:
            self.encoder_fc_in = nn.Linear(input_dim, hidden_width).to(self.device)
            ...
            for ii in range(1, hidden_depth):
                self.encoder_fc_hid.append(nn.Linear(hidden_width, hidden_width).to(self.device))
            ...
            self.encoder_fc_out = nn.Linear(hidden_width, output_dim).to(self.device)
            self.opt_parameters_encoder.append(self.encoder_fc_out.weight)
            self.opt_parameters_encoder.append(self.encoder_fc_out.bias)

        else:
            self.encoder_fc_out = nn.Linear(input_dim, output_dim).to(self.device)
            self.opt_parameters_encoder.append(self.encoder_fc_out.weight)
            self.opt_parameters_encoder.append(self.encoder_fc_out.bias)
    ...
    #         z = np.concatenate(
    #             (
    #                 np.ones((x.shape[0], first_obs_const)),
    #                 x[:, :n],
    #                 self.encode_forward_(x_t).detach().numpy(),
    #             ),
    #             axis=1,
    #         )
    #     else:
